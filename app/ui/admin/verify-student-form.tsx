...
import { z } from 'zod';
import { StudentVerifySchema } from '@/app/lib/zod-schemas';

type FormValues = z.infer<typeof StudentVerifySchema>;

export default function VerifyStudentForm({ registrationNumber }: { registrationNumber: string }) {
  ...
  const onSubmit = async (data: FormValues) => {
    try {
      const response = await fetch('/api/send-ticket-admin', {
        method: 'POST',
        headers: {
  ...
  return (
    <form onSubmit={handleSubmit(onSubmit)}>
      <div className="mb-4">
        <label htmlFor="ticket_number" className="block text-gray-400 mb-2 text-sm sm:text-base">
          Ticket number
        </label>
        <Controller
          name="ticket_number"
            ...
            <>
              ...
              {errors.ticket_number && (
                <div className="flex gap-2 items-center mt-2">
                  ...
                  <p className="text-red-600 text-sm">{(errors.ticket_number as FieldError)?.message || 'An error occurred'}</p>
                </div>
              )}
            </>
          )}
